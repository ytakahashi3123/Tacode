...
| `test_general_math.py` | The two vector helpers `general.py` keeps for speed, `cross_product` and `vector_norm`, return bit for bit what `np.cross` and `np.linalg.norm` return, over random vectors spanning twenty-four decades, and the basis vectors come out right-handed |
...
**The Knudsen axis of a table belongs to the length it was built with**, since
`Kn = λ/L`, and that same length normalises the moment coefficients. A table may declare
it with a `# Reference length: 0.8 m` line in its header; when it does and
`satellite.characteristic_length` differs, the run says so at start-up, naming both
values, and carries on — applying a table to another body is a legitimate approximation,
and the re-entry tutorials do exactly that. `database/aerodynamic/README.md` lists the
length of each shipped table, how the one of `aerodynamic.txt` was recovered from the
table itself, and what the mismatch in the tutorials is worth.
...
interpolators are. It is a hand-written multilinear interpolation rather than
`RegularGridInterpolator`, for the same reason the aerodynamic table is: one point in
three dimensions costs 39 us through scipy and 4 us written out, which was half the run
time of the tutorial that reads a table. It returns bit-for-bit what scipy returns, and
is held to that against scipy itself.
