...
\begin{tabbing}
\hspace{7.0em}\= \hspace{1.5em} \= \hspace{11cm} \kill
...
\textbf{table against the total angle of attack as well as the Knudsen number} gives the
six coefficients $C_{Fx}$, $C_{Fy}$, $C_{Fz}$, $C_{Mx}$, $C_{My}$, $C_{Mz}$ needed for six
degrees of freedom; it is organised in blocks introduced by a line \texttt{AOA <value>}, and a file
...
\begin{table}[h]
\caption{The test suite, grouped by what it establishes. The \texttt{test\_} prefix of every module name is omitted.}
\label{test_modules}
\begin{center}
\small
...
\hline \hline
Window & $q_\infty$ Tacode & $q_\infty$ required & ratio &
$\omega$ computed & $\omega$ measured & ratio \\
s & N/m$^2$ & N/m$^2$ & & rad/s & rad/s & \\
\hline
