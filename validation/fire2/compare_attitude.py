...
#
#         になる（Murphy BRL-1216 の tricyclic 解を機体軸へ戻したもの）。2 つのモードの
#         うち**遅いほう**で、動圧 0 では lambda = p になる。これは「空間に固定された
#         迎角を、回っている機体から見れば毎秒 p だけ回って見える」というだけの意味で、
#         無トルクのときの章動（機体軸で (Ix - Iy) p / Iy = 4.5 rad/s）とは別のモード。
